...
layout(location = 6) uniform vec4 blob1;
layout(location = 7) uniform vec4 blob2;
layout(location = 8) uniform vec4 blob3;
layout(location = 9) uniform vec4 blob4;
layout(location = 10) uniform vec4 blob5;
layout(location = 11) uniform vec4 blob6;
layout(location = 12) uniform vec4 blob7;
layout(location = 13) uniform vec4 blob8;

// Per blob: x: borderRadius, y: type, z: unused, w: unused
layout(location = 14) uniform vec4 blobParams1;
layout(location = 15) uniform vec4 blobParams2;
layout(location = 16) uniform vec4 blobParams3;
layout(location = 17) uniform vec4 blobParams4;
layout(location = 18) uniform vec4 blobParams5;
layout(location = 19) uniform vec4 blobParams6;
layout(location = 20) uniform vec4 blobParams7;
layout(location = 21) uniform vec4 blobParams8;

// ----------------------------
// Fill type
// ----------------------------

vec4 getFill(vec2 p, float fillType, vec4 color, vec4 color2, vec4 color3) {
    if (fillType < 0.5) {
        return color;
    } else if (fillType < 1.5) {
        // Linear gradient with 3 colors
        if (p.y < 0.5) {
            return mix(color, color2, p.y * 2.0);
        } else {
            return mix(color2, color3, (p.y - 0.5) * 2.0);
        }
    } else {
        // Radial gradient with 3 colors
        float dist = length(p - vec2(0.5, 0.5)) * 2.0;
        if (dist < 0.5) {
            return mix(color, color2, dist * 2.0);
        } else {
            return mix(color2, color3, (dist - 0.5) * 2.0);
        }
    }
}
...
void main() {
    ...
    vec4 fillColor = getFill(p, uParams.w, uColor, uColor2, uColor3);
